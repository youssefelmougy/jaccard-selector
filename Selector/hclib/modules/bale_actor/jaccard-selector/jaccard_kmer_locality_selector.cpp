...
* 
 *****************************************************************/ 
/*! \file jaccard_kmer_locality_selector.cpp
 * \brief Demo application that calculates jaccard similarity for k-mer sequence graph
 *          (Uses a locality-aware approach to communication and computation, discussed in Section 5.1)
 *
    ...
    void req_process(JaccardPkt pkg, int sender_rank) {
        ...
        for (int64_t uk = mat_->loffset[pkg.index_u]; uk < mat_->loffset[pkg.index_u+1]; uk++) {
            if (pkg.x == mat_->lnonzero[uk]) {
                ...
                if (pkg.pos_row % THREADS == MYTHREAD) {
                    int pos = 0;
                    for (int i = intersection_mat_->loffset[pkg.pos_row / THREADS]; i < intersection_mat_->loffset[(pkg.pos_row / THREADS) + 1]; i++) {
                        if (pos == pkg2.pos_col) {
        ...
        for (int64_t v = 0; v < A2->lnumrows; v++) {             //vertex v (local)
            for (int64_t k = A2->loffset[v]; k < A2->loffset[v+1]; k++) {
                ...
                for (int64_t i_rows = row_num; i_rows < A2->numrows; i_rows++) {
                    ...
                    if (i_rows % THREADS == MYTHREAD) {
                        JaccardPkt pkg2;
                        for (int64_t uk = A2->loffset[i_rows / THREADS]; uk < A2->loffset[(i_rows / THREADS) +1]; uk++) {
                            if (v_nonzero == A2->lnonzero[uk]) {
                                pkg2.pos_row = i_rows;
                                pkg2.pos_col = row_num;

                                if (i_rows % THREADS == MYTHREAD) {
                                    int pos = 0;
                                    for (int i = Jaccard_mat->loffset[i_rows / THREADS]; i < Jaccard_mat->loffset[(i_rows / THREADS) + 1]; i++) {
                                        if (pos == pkg.pos_col) {
                                    ...
                                    jacSelector->send(RESPONSE, pkg2, i_rows % THREADS);
                                }
                            }
                        }
...
int main(int argc, char* argv[]) {
    ...
    hclib::launch(deps, 2, [=] {
        ...
        T0_fprintf(stderr, "\nRunning Jaccard Similarity K-mers, locality-aware (selector): \n");

        double laptime_jaccard = 0.0;
        
        PAT_region_begin(1, "selector_jaccard_localityaware"); // region begin for HWPC evaluation using CrayPat

        // Running selector model for jaccard similarity on k-mer matrix (locality-aware)
